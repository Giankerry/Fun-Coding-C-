#include <iostream>/*This are for input,output and strings*/
#include <string.h>
#include <stdio.h>
#include <string>
#include <windows.h>/*This is for the text color  function to work*/
#include <chrono>/*This is for the function chrono*/
#include <time.h>/*This is used to make use of system clock for assigning values for the random numbers.I used this for the hit-rate(rand) function*/
#include <unistd.h>/*Used for sleep function*/
#include <cstdlib>/*Used for some unique functions used*/
using namespace std;
#define hit "You Died...";
/*Here are the variables used for the game*/
...
string enter1;
string enter2;
/*I used multiple arrays for the dialogue part so it feels a bit more like someone is telling a story.I used for loops to add a delay for each line*/
string dialogue[] = {
...
/*This function is used to assign the variable color for changing the "color" of specific lines in the text*/
...
/*Here is the role and item selection where I used multiple switch & case statements for the choices*/
system("cls");
...
/*Series of arithmetic operators were used for the stats*/
hpbase += 300;
energy+=4;
maxhp+= 300;
atkbase += 25;
...
cin >> weap;
/*This two functions makes the current input to be ignored by the system and read the next input instead.This is used to avoid the system to be confused of input values*/
cin.clear();
cin.ignore();
...
break;
/*If Else statements were used for assessing different conditions*/
if(role>=1&&role<=4){
...
/*This are the loops for the dialogue output where it shows strings from the array one by one with delay.*/
...
SetConsoleTextAttribute(color,6);
    ...
    pchance = 1;
    /*This srand function serves as a randomizer so there will be a random value everytime the for loop repeats*/
    srand((unsigned int)time(NULL));
    /*The time(NULL)--system clock is used to assign a different value as the time changes*/
...
for (int i = 0; i < sizeof(conclusionA)/sizeof(conclusionA[0]); i++ ) {
    ...
    sleep(3);}
cout<<"**************************************************************************************"<<endl;
cout<<"Mission has been completed.\nYou Won! Thank you for playing the game."<<endl<<endl;
cout<<"Creator:Gian Kerry Vino"<<endl;
cout<<"**************************************************************************************"<<endl;
exit(EXIT_SUCCESS);}
}
case(2):
...
for (int i = 0; i < sizeof(ending)/sizeof(ending[0]); i++ ) {
    ...
    sleep(3);}
cout<<"**************************************************************************************"<<endl;
cout<<"The first story has been completed!\nThank you for playing the game."<<endl<<endl;
cout<<"Creator:Gian Kerry Vino"<<endl;
cout<<"**************************************************************************************"<<endl;
exit(EXIT_SUCCESS);
}
}
/*This contains the initial stages of the game*/
...
system ("cls");
SetConsoleTextAttribute(color,7);
/*This serves as the loading screen*/
	cout<<"Press 1 to continue..."<<endl;
cin>>cont;
	cout << "Proceeding to next point";
	for (int d=0;d<3;d++){
